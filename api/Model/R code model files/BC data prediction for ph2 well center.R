...
attribute <- c('well_authority_number','uwi_formatted','geom',
               ...
               'cumulative_condensate_production_bbl','completion_bottom_depth_m',
               'prod_ip3_oil_bbld','prod_ip3_gas_mcfd',
               'full_status',
               'total_drilled_depth_m',
...
#Rename columns
colnames(testdata)[colnames(testdata) == "well_authority_number"] <- "Licence"
...
colnames(testdata)[colnames(testdata) == "completion_bottom_depth_m"] <- "Completion.Bottom..mKB."
...
colnames(testdata)[colnames(testdata) == "full_status"] <- "Full.Status"
colnames(testdata)[colnames(testdata) == "total_drilled_depth_m"] <- "True.Vertical.Depth..m.TVD."
...
colnames(testdata)[colnames(testdata) == "cumulative_marketable_gas_production_mcf"] <- "Cumulative.Marketable.Production..e3m3."
...
for (i in 1:length(loc)){       
  ...
  a$Cumulative.Marketable.Production..e3m3. = sum(b$Cumulative.Marketable.Production..e3m3.,na.rm=TRUE)
...
prod=c("Cum..Gas.Production..e3m3.","Cum..Condensate.Production..bbl.",
       ...
       "Initial.3Mth.Prod..Gas..Mcf.d.","Cumulative.Marketable.Production..e3m3.")#If any attributes not applicable in CARTOFACT for the data, delete it here.
...
nume <- testdata[,c("True.Vertical.Depth..m.TVD.",
                    ...
                    "Initial.3Mth.Prod..Gas..Mcf.d.","Cumulative.Marketable.Production..e3m3.")]
...
var_nume=c("Spud.Date","Last.Production.Date","Cum..Oil.Production..m3.",
           "Cum..Water.Production..m3.","Initial.3Mth.Prod..Oil..BBL.d.",
           "Last.3Mth.Prod..Oil..BBL.d.","Last.3Mth.Prod..Water..BBL.d.",
           "Initial.3Mth.Prod..BOE..BOE.d.",
           "Cumulative.Marketable.Production..e3m3.")# select all the numerical attributes will be used in model
...
lognume <- as.data.frame(lognume)
colnames(lognume) <- c("Spud.Date")
...
colnames(logdata_test1) <- c("Spud.Date")
# logdata_test1 <- as.data.frame(cbind(lognume_test1,logcate_test))

# logcate<-cate[,c("LLR.Abandonment.Area")] # add "Well.Type.Final" when obtain the data
# logcate <- as.data.frame(logcate)
# colnames(logcate) <- c("LLR.Abandonment.Area")# add "Well.Type.Final" when obtain the data
...
Pred_result_BC_ph2W_P<- Pred_result

Pred_result_BC_ph2W_P <- as.data.frame(Pred_result)

Pred_result_BC_ph2W1.1 <- as.data.frame(Pred_result_BC_ph2W_P)
for(i in 1:dim(Pred_result_BC_ph2W_P)[1]){
  if(Pred_result_BC_ph2W_P$Well.Center.Pass.Probability[i] >= 0.90){
    Pred_result_BC_ph2W1.1$Well.Center.Pass.Probability[i] <- "Pass"
  }
  else{
    Pred_result_BC_ph2W1.1$Well.Center.Pass.Probability[i] <- "Fail"
  }
}
...
# 
# 
# fullresults=read.csv("Phase 1 Well Centre Dataset.csv",as.is=TRUE, header=TRUE)
# 
...
# }
# 
# fullresults$New.Status.TH0.90 <- NA
# #AB results
# for(i in 1:dim(AB_pred_result1.1)[1]){
#   loc = which(AB_pred_result1.1$Licence[i]==fullresults$Licence.Number)
#   fullresults$New.Status.TH0.90[loc]=AB_pred_result1.1$Well.Center.Pass.Probability[i]
# }
# 
# #SK results
# for(i in 1:dim(Pred_result_SK_ph2W1.1)[1]){
#   loc = which(Pred_result_SK_ph2W1.1$Licence[i]==fullresults$Licence.Number)
#   fullresults$New.Status.TH0.90[loc]=Pred_result_SK_ph2W1.1$Well.Center.Pass.Probability[i]
# }
# 
# #BC results
# for(i in 1:dim(Pred_result_BC_ph2W1.1)[1]){
#   loc = which(Pred_result_BC_ph2W1.1$UWI[i]==fullresults$UWI)
#   fullresults$New.Status.TH0.90[loc]=Pred_result_BC_ph2W1.1$Well.Center.Pass.Probability[i]
# }
# 
# 
# 
# 
# fullresults$New.Status[fullresults$New.Status == 1] <- "Pass"
# fullresults$New.Status[fullresults$New.Status == 0] <- "Fail"
