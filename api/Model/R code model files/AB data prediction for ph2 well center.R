...
py_run_file("/Users/qianhuang/Desktop/360/model/extracting attributes/extracting cartofact attributes.py")
testdata <- read.csv("/Users/qianhuang/Desktop/360/model/extracting attributes/attribute_df.csv")



# obtain attribute 'cumulative_marketable_production_e3m3', 'remaining_energy_terajoules'
search_id <- c('field_name')
id <- unique(testdata$field_name)
id <- data.frame(id = id)
write_xlsx(id,"/Users/qianhuang/Desktop/360/model/extracting attributes/id.xlsx")

attribute <- c('field_name','cumulative_marketable_production_e3m3','remaining_energy_content_terajoules')
table <- c('live_well_ab_st98_field_gas_reserve')
at_table <- as.data.frame(cbind(attribute,table,search_id))
write_xlsx(at_table,"/Users/qianhuang/Desktop/360/model/extracting attributes/at_table.xlsx")
py_run_file("/Users/qianhuang/Desktop/360/model/extracting attributes/extracting attributes other table.py")
remainenergy <- read.csv("/Users/qianhuang/Desktop/360/model/extracting attributes/attribute_ot.csv")
testdata <- merge(testdata,remainenergy,by = 'field_name',all.x = TRUE)
...
write_xlsx(at_table,"/Users/qianhuang/Desktop/360/model/extracting attributes/at_table.xlsx")
py_run_file("/Users/qianhuang/Desktop/360/model/extracting attributes/extracting attributes other table.py")
oilinplace <- read.csv("/Users/qianhuang/Desktop/360/model/extracting attributes/attribute_ot.csv")
testdata <- merge(testdata,oilinplace,by = 'field_name',all.x = TRUE)


# obtain attribute 'volume_recovered1'
search_id <- c('geom')
id <- as.data.frame(testdata$geom)
names(id) <- 'id'
write_xlsx(id,"/Users/qianhuang/Desktop/360/model/extracting attributes/id.xlsx")

attribute <- c('geom','volume_recovered_1')
table <- c('live_incidents_aer_releases')
at_table <- as.data.frame(cbind(attribute,table,search_id))
write_xlsx(at_table,"/Users/qianhuang/Desktop/360/model/extracting attributes/at_table.xlsx")
py_run_file("/Users/qianhuang/Desktop/360/model/extracting attributes/extracting attributes other table.py")
volrecover <- read.csv("/Users/qianhuang/Desktop/360/model/extracting attributes/attribute_ot.csv")
testdata <- merge(testdata,volrecover,by = 'geom',all.x = TRUE)
testdata$volume_recovered_1[is.na(testdata$volume_recovered_1)] <- 0
...
colnames(testdata)[colnames(testdata) == "remaining_energy_content_terajoules"] <- "Remaining.Energy.Content..TJ."
colnames(testdata)[colnames(testdata) == "volume_recovered_1"] <- "Volume.Recovered.1"
colnames(testdata)[colnames(testdata) == "cumulative_marketable_production_e3m3"] <- "Cumulative.Marketable.Production..e3m3."
colnames(testdata)[colnames(testdata) == "volume_recovered_1"] <- "a"
...
prod=c("Cum..Oil.Production..m3.","Cum..Water.Production..m3.",
       ...
       "Cum..Condensate.Production..bbl.",
       "Cum..Gas.Production..e3m3.",
       "Cumulative.Marketable.Production..e3m3.")#If any attributes not applicable in CARTOFACT for the data, delete it here.
for (i in 1:length(prod)){
  testdata[prod[i]][is.na(testdata[prod[i]])] <- 0
}

#Run the following line if "Oil in Place" is available in CARTOFACT
prod=c("Remaining.Energy.Content..TJ.","Oil.In.Place..e3m3.")
for (i in 1:length(prod)){
  testdata[prod[i]][is.na(testdata[prod[i]])] <- 0
}
...
date <- testdata[,c("Spud.Date","Last.Production.Date","Surface.Aband..Date")] 
date <- mutate_all(date, function(x) as.numeric(as.character(x)))

# nume <- testdata[,c("Deepest.Form..Depth..mKB.","Total.Well.Depth..mKB.","True.Vertical.Depth..m.TVD.","Completion.Length..m.","Base.Of.Groundwater.Protection.Elev...m.above.sea.level.",
...
#                   "Volume.Released.1","Oil.In.Place..e3m3.")]

nume <- testdata[,c("Total.Well.Depth..mKB.","True.Vertical.Depth..m.TVD.",
                  "Completion.Bottom..mKB.","Cum..Gas.Production..e3m3.",
                  "Cum..Water.Production..m3.","Cum..Oil.Production..m3.","Ground.Elevation..m.above.sea.level.",
                  "Initial.3Mth.Prod..Gas..Mcf.d.","Initial.3Mth.Prod..Oil..BBL.d.","Last.3Mth.Prod..Oil..BBL.d.",
                  "Last.3Mth.Prod..Water..BBL.d.","Initial.3Mth.Prod..BOE..BOE.d.",
                  "Cum..Condensate.Production..bbl.",
                  "Volume.Recovered.1",
                  "Remaining.Energy.Content..TJ.","Oil.In.Place..e3m3.","Cumulative.Marketable.Production..e3m3.")]
...
nume.gasfield <- testdata[,c("Cumulative.Marketable.Production..e3m3.",
                             "Remaining.Energy.Content..TJ.")]

#un-comment the following line if "Oil in Place" is available from CARTOFACT
nume.oil <- testdata[,c("Oil.In.Place..e3m3.")]


#If "Oil in Place" is available from CARTOFACT, use the following line
...
var_nume=c("Spud.Date","Remaining.Energy.Content..TJ.", "Cum..Oil.Production..m3.",
           "Cum..Water.Production..m3.","Initial.3Mth.Prod..Oil..BBL.d.","Last.3Mth.Prod..Oil..BBL.d.",
           "Last.3Mth.Prod..Water..BBL.d.","Initial.3Mth.Prod..BOE..BOE.d.",
           "Volume.Recovered.1","Cumulative.Marketable.Production..e3m3.",
           "Oil.In.Place..e3m3.")# select all the numerical attributes will be used in model
...
Data<- Data %>% distinct(UWI, Phase.2.ESA.Well.Centre, .keep_all = TRUE)
dup <- subset(Data,duplicated(UWI))
...
AB_pred_result1.1 <- as.data.frame(AB_pred_result_P)
for(i in 1:dim(AB_pred_result_P)[1]){
  if(!(is.na(AB_pred_result_P$Well.Center.Pass.Probability[i]))){
    if(AB_pred_result_P$Well.Center.Pass.Probability[i] >= 0.90){
      AB_pred_result1.1$Well.Center.Pass.Probability[i] <- "Pass"
    }
    else{
      AB_pred_result1.1$Well.Center.Pass.Probability[i] <- "Fail"
    }
  }
}
...
# fullresults=read.csv("Testing data results Oct26.csv",as.is=TRUE, header=TRUE)
# 
# for(i in 1:dim(AB_pred_result_P)[1]){
#   loc = which(AB_pred_result_P$Licence[i]==fullresults$Licence)
#   fullresults$RF[loc]=AB_pred_result_P$Well.Center.Pass.Probability[i]
# }
# 
# pdflist['Amber.s.predicted.results'] <- NA
# for(i in 1:dim(AB_pred_result1.1)[1]){
#   loc = which(AB_pred_result1.1$Licence[i]==pdflist$License)
#   pdflist$Amber.s.predicted.results[loc]=AB_pred_result1.1$Well.Center.Pass.Probability[i]
# }
# 
# View(fullresults)
# 
# 
# pdflist["Original"] <- NA
# pdflist["Sentiment"] <- NA
# for(i in 1:dim(sentiment)[1]){
#   loc = which(sentiment$PDF.Name[i]==pdflist$PDF.Name)
#   pdflist$Original[loc]=sentiment$Original[i]
#   pdflist$Sentiment[loc]=sentiment$Sentiment[i]
# }
